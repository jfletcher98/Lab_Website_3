...
app.get('/player_info/post', function(req, res) {
	...
	var query2 = "select * from football_players where id = " + splayerid + ";";
	var query3 = "SELECT COUNT(*) FROM football_games WHERE " + splayerid + " = ANY(players);";
	db.task('get-everything', task => {
        return task.batch([
            task.any(query1),
            task.any(query2),
    ...
    .then(pinfo => {
    	res.render('pages/player_info',{
				thisTitle: "Football Player Page",
				idName: pinfo[0],
				thisData: pinfo[1],
				game_n: pinfo[2]
			})
